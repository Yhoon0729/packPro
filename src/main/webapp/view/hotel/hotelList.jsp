...
<body>
	<div class="container">
		...
		<div class="button-container text-center mb-4">
			<c:forEach var="h" items="${locli}">
				<button class="filter-button btn btn-light"
					onclick="filterTable('${h.fstLoc}')">${h.fstLoc}</button>
			</c:forEach>
		...
		<table id="hoteltable" class="table table-striped">
			...
			<tbody>
				<c:forEach var="h" items="${li}">
					<tr data-toggle="modal" data-target="#hotelModal"
						onclick="showModal(this, 'https://lh3.googleusercontent.com/p/AF1QipOZxVnZ05C7IAOHgd9bQnAEmN9FxbJ6X-QuykNN=s680-w680-h510')">
						<td>${h.htlId}</td>
						<td>${h.fstLoc}</td>
						<td>${h.htlName}</td>
						<td>${h.htlGrade}</td>
	...
	<!-- Modal -->
	<form action="/packagePro/car/rentCar">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="hotel-info">
							...
							<p>
								<strong>호텔가격:</strong> <span id="modalHtlPrice"></span>
							</p>
						</div>
						<div class="booking-info">
							<form id="bookingForm" method="post" action="hotelBookingInsert">
								<div class="form-group">
									<label for="checkinDate">체크인 날짜</label> <input type="date"
										class="form-control" id="checkinDate" name="checkinDate"
										required>
								</div>
								<div class="form-group">
									<label for="checkoutDate">체크아웃 날짜</label> <input type="date"
										class="form-control" id="checkoutDate" name="checkoutDate"
										required>
								</div>
								<div class="form-group">
	...
	<script>
		...
		function submitBooking() {
			const hotelName = document.getElementById('modalHtlName').textContent;
			...
			const roomCount = document.getElementById('rooms').value;
			const totalPrice = calculateTotalPrice(); // 총 가격 계산 로직 추가

			if (checkinDate && checkoutDate && adultCount && childCount
					&& roomCount) {
				window.location.href = `confirmBooking.jsp?hotelName=${hotelName}&checkinDate=${checkinDate}&checkoutDate=${checkoutDate}&adultCount=${adultCount}&childCount=${childCount}&roomCount=${roomCount}&totalPrice=${totalPrice}`;
			} else {
				alert('모두 다 작성해주세요!');
			}
		}
		...
		function calculateTotalPrice() {
			const pricePerRoom = 100000; // 예시 가격, 실제 가격 계산 로직 필요
			const roomCount = document.getElementById('rooms').value;
